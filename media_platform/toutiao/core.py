...
class ToutiaoCrawler(AbstractCrawler):
    ...
    def __init__(self):
        ...
        self.search_url = "https://so.toutiao.com"
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            ...
            if not await self.toutiao_client.pong():
                utils.logger.info("[ToutiaoCrawler] 需要登录，启动登录流程...")
            ...
            if config.CRAWLER_TYPE == "search":
                ...
                utils.logger.info("[ToutiaoCrawler.start] 仅支持search爬取类型")
            
            utils.logger.info("[ToutiaoCrawler.start] 今日头条爬虫完成")
    
    async def search(self):
        """搜索爬取 - 参考小红书搜索逻辑，支持多页爬取"""
        utils.logger.info("[ToutiaoCrawler.search] 开始搜索今日头条关键词")
        
        # 每页返回的结果数（今日头条搜索每页约15条）
        page_size = 15
        ...
        for keyword in config.KEYWORDS.split(","):
            ...
            while (page - start_page + 1) * page_size <= max_notes:
                ...
                try:
                    utils.logger.info(f"[ToutiaoCrawler.search] 搜索关键词: {keyword}, 第 {page} 页")
                    
                    # 搜索并获取结果（传入页码）
                    posts = await self.toutiao_client.search_posts(keyword=keyword, page=page)
                    
                    if not posts:
                        utils.logger.info(f"[ToutiaoCrawler.search] 第 {page} 页没有更多结果，停止翻页")
                        break
                    
                    utils.logger.info(f"[ToutiaoCrawler.search] 第 {page} 页获取到 {len(posts)} 条结果")
                    
                    for post in posts:
                        # 保存到数据库
                        await toutiao_store.update_toutiao_post(post, keyword)
                        total_collected += 1
                    ...
                    utils.logger.info(f"[ToutiaoCrawler.search] 第 {page-1} 页完成，休眠 {config.CRAWLER_MAX_SLEEP_SEC} 秒")
                    
                except Exception as e:
                    utils.logger.error(f"[ToutiaoCrawler.search] 第 {page} 页搜索失败: {e}")
                    break
            
            utils.logger.info(f"[ToutiaoCrawler.search] 关键词 '{keyword}' 爬取完成，共 {total_collected} 条")
    
    async def create_toutiao_client(self, httpx_proxy: Optional[str]) -> ToutiaoClient:
        """创建API客户端"""
        utils.logger.info("[ToutiaoCrawler.create_toutiao_client] 创建今日头条API客户端...")
